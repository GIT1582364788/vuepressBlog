...
使用javascript也可以做后端开发，只不过需要借助一个node.js。一门编程语言编写好之后，需要在运行环境中执行。如果把js代码跑在浏览器里边，浏览器本身就是一个运行环境，所以javascript可以跑在浏览器里做前端开发。 如果把js代码运行在node中，这里node也是一个运行环境，是一个后端的运行环境，这样就可以用javascript做后端开发。
